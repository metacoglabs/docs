---
title: "Changelog"
description: "SDK and API releases: metering, verbs, latency improvements—skim before you upgrade."
...
<CardGroup cols={2}>
  <Card title="Async client" icon="bolt">
    Native async methods on a parallel `AsyncTex` class. Same API surface, awaitable. Targeting Q3.
  </Card>
  ...
  <Card title="Scoped API keys" icon="key">
    `read` / `write` / `usage:read` scopes for principle-of-least-privilege keys.
  </Card>
</CardGroup>
