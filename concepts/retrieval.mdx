...
Defaults: **15** in `active` mode, **25** in `deep` mode. Server caps at **30** regardless of the value you send. Sensible explicit values:

| Use case | `top_k` |
| --- | --- |
| Live chat — small context window | 3–5 |
| Live chat — large context window | 8–15 |
| Summaries / digests | 20–30 |

Larger `top_k` → more `tokens_out` billed. Send a smaller `top_k` if you only need a handful of hits.
...
`timeline` is an `Optional[str]` — a pre-rendered chronological summary the server walks the temporal index to build. Treat it as a paragraph to drop into a prompt, not a structured array. Useful for "when did the user first mention X?" queries.
