...
- **Refresh.** `POST /auth/refresh` returns a new access *and* refresh token. The refreshed token keeps the `act` claim (still locked to the same user) but does not carry the key's roles or the session id.
- **Revoking a key** stops new token exchanges. It does **not** invalidate tokens already issued: access tokens stay valid until they expire, and `/auth/refresh` does not check whether the originating key is still active.
- There is no per-token revocation endpoint today. To limit exposure, don't hand refresh tokens to untrusted clients — have your server re-exchange instead.

## Rotating keys

<Steps>
  <Step title="Mint key B">
    [Dashboard → API Keys → New key](https://app.getmetacognition.com/dashboard/keys), or `POST /me/api-keys` with an org-admin token.
  </Step>
  <Step title="Roll out">
