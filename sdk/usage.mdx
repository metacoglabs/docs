...
<AccordionGroup>
  <Accordion title="Per-response usage" icon="receipt">
    Every `remember` and `recall` carries `Usage` directly on the response — no extra round-trip:
  ...
  <Accordion title="Quota-aware routing" icon="route">
    ...
    ```
    Cache `tex.usage.today()` for a minute or two on your side — this is just for soft gating.
  </Accordion>
  <Accordion title="Build a usage chart" icon="chart-bar">
    Use `summary(month="YYYY-MM")` for the last 6 months → render with Plotly, Recharts, or whatever you use.
  </Accordion>
</AccordionGroup>
