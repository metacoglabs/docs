...
- **Refresh-proof** — Streamlit's session state dies on refresh; Tex doesn't.
- **No glue code** — no Postgres / Redis layer for chat history.
- **Built-in debug panel** — the expander shows exactly which memory items the model saw.
